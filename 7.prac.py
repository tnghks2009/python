...
df_line_melted = df_line_melted.groupby(['호선', '시간'], as_index=False)[['인원수']].sum().reset_index(drop=True)
...
df_sta_melted = df_sta_melted.groupby('시간', as_index=False)[['인원수']].sum().reset_index(drop=True)
...
text = alt.Chart(df_sta_melted).mark_text(dx=0, dy=0, color='black').encode(x='시간', y='인원수', text=alt.Text('인원수'))
st.caption(option)
st.altair_chart(chart + text, use_container_width=True)


# 파일실행: File > New > Terminal(anaconda prompt) - streamlit run 7.prac.py